...
```python
SCHEMA_PATH = PM.engineering
TRAIN_DATASET_FILE = PM.imputed_file
...
```python
_finder = DragonArtifactFinder(directory=PM.autoencoder, load_scaler=True, load_schema=False, strict=True)

encoder = DragonAutoencoder.from_artifact_finder(_finder)
```
...
trainer.evaluate(model_checkpoint="best",
                ...
                test_format_configuration=ChosenMetricsConfig(real_color="tab:brown", gen_color="teal"),
                )
```
